...
from stockcentral.build_data import GRILON3_PANTONE_CACHE
...
def build_pantone_cache(timeout_seconds: int = 4, max_workers: int = 12) -> dict[str, str]:
    ...
    return {
        pantone_cache_key(product.title): product.pantone
        for _, product in sorted(enriched.items())
        if product.pantone
    }


def write_pantone_cache(
    output_path: str | Path = GRILON3_PANTONE_CACHE,
    timeout_seconds: int = 4,
    max_workers: int = 12,
) -> dict[str, str]:
    cache = build_pantone_cache(timeout_seconds=timeout_seconds, max_workers=max_workers)
...
def pantone_cache_key(title: str) -> str:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Actualiza la cache local de Pantone desde fichas Grilon3.")
    parser.add_argument("--output", default=str(GRILON3_PANTONE_CACHE))
    parser.add_argument("--timeout-seconds", type=int, default=4)
    parser.add_argument("--max-workers", type=int, default=12)
    args = parser.parse_args()

    cache = write_pantone_cache(
        output_path=args.output,
        timeout_seconds=args.timeout_seconds,
        max_workers=args.max_workers,
    )
    print(f"Pantone cache actualizada: {len(cache)} productos")
